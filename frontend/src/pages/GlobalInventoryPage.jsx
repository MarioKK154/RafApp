...
function GlobalInventoryPage() {
    ...
    const canManageInventory =
        user && (['admin', 'project manager'].includes(user.role) || user.is_superuser);
    ...
    }, [debouncedSearch, buildCatalogParams, t]);
    ...
    return (
        <div className="container mx-auto p-4 md:p-8 max-w-7xl animate-in fade-in duration-500">
                ...
                <div className="xl:col-span-8 relative group">
                    ...
                    <input
                        ...
                        placeholder={t('catalog_search_full_db', {
                            defaultValue: 'Search the full catalog (names, SKUs, cable codes…)…',
                        })}
                        value={searchTerm}
            ...
            {!debouncedSearch.trim() && (
                <p className="mb-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                    {t('global_inventory_quick_browse_hint', {
                        defaultValue:
                            'Type above to search the entire database; below is a short sample with your supplier filters.',
                    })}
                </p>
            )}

            <div className="mb-10 flex flex-wrap gap-6 items-center text-[10px] font-black uppercase tracking-widest text-gray-400">
                <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[1.25rem] px-6 py-4 flex items-center gap-3 shadow-sm">
                    <AdjustmentsHorizontalIcon className="h-4 w-4 text-indigo-500" />
                    <span className="text-gray-900 dark:text-gray-100">
                        {items.length}{' '}
                        {debouncedSearch.trim()
                            ? t('results', { defaultValue: 'matches' })
                            : t('items_sample', { defaultValue: 'items (sample)' })}
                    </span>
                </div>
            </div>
                                    ...
                                    <img
                                        ...
                                        onError={(e) => {
                                            ...
                                            e.currentTarget.src = resolveImageUrl(
                                                '/static/inventory_images/uncategorized.png',
                                            );
                                        }}
