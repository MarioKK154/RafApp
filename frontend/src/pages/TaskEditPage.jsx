...
function TaskEditPage() {
        ...
        if (!authIsLoading && isAuthenticated && taskId) {
            ...
            try {
                ...
                if (fetchedTask.project_id) {
                    // --- THIS IS THE FIX ---
                    // Changed limit from 1000 to 200 to match backend validation rule.
                    const allTasksInProjectRes = await axiosInstance.get(`/tasks/?project_id=${fetchedTask.project_id}&limit=200`);
                    allTasksInProject = allTasksInProjectRes.data;
                    predecessorOptions = allTasksInProject
                ...
                const selectedPredecessors = (fetchedTask.predecessor_ids || []).map(id => {
                    const task = allTasksInProject.find(t => t.id === id);
                    return { value: id, label: task ? task.title : `Task ${id}` };
                });
                ...
                const errorMsg = err.response?.status === 404 ? 'Task not found.' : 'Failed to load task data.';
                setError(errorMsg);
    ...
    }, [taskId, isAuthenticated, authIsLoading]);
    ...
    const handleSubmitUserDetails = async (e) => {
        ...
        try {
            ...
            const addPromises = dependenciesToAdd.map(id => axiosInstance.post(`/tasks/${taskId}/dependencies`, { predecessor_id: id }));
            const removePromises = dependenciesToRemove.map(id => axiosInstance.delete(`/tasks/${taskId}/dependencies/${id}`));
    ...
    const handleCommissionTask = async () => {
        if (!canCommissionTask || !taskData || taskData.status !== "Done" || taskData.is_commissioned) {
            toast.warn("Task must be 'Done' and not yet commissioned to perform this action.");
            return;
        }
        ...
        try {
            ...
            toast.error(err.response?.data?.detail || 'Failed to commission task.');
        } finally { 
    ...
    return (
        <div className="container mx-auto p-4 md:p-6">
            <h1 className="text-2xl font-bold mb-2">Edit Task: {taskData?.title}</h1>
            
            <p className="text-sm mb-4 text-gray-500">
                Current Status: <span className="font-semibold">{taskData?.status}</span>
                {taskData?.is_commissioned && (
                    <span className="ml-2 px-2 py-0.5 text-xs font-semibold text-green-800 bg-green-200 rounded-full">Commissioned</span>
                )}
            </p>
            ...
            <fieldset disabled={!canManageTasks || isSubmitting} className="mb-8">
                <form onSubmit={handleSubmitUserDetails} className="space-y-4 max-w-lg bg-white dark:bg-gray-800 p-6 rounded shadow-md">
                    <div>
                        ...
                        <select name="project_id" id="project_id" required value={formData.project_id} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md">
                            {projects.map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="title">Task Title <span className="text-red-500">*</span></label>
                        <input type="text" name="title" id="title" required value={formData.title} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md"/>
                    </div>
                    <div>
                        <label htmlFor="description">Description</label>
                        <textarea name="description" id="description" rows="3" value={formData.description} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md"></textarea>
                    </div>
                    <div>
                        <label htmlFor="status">Status</label>
                        <select name="status" id="status" required value={formData.status} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md">
                            {EDITABLE_TASK_STATUSES.map(s => (<option key={s} value={s}>{s}</option>))}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="priority">Priority</label>
                        <select name="priority" id="priority" required value={formData.priority} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date" value={formData.start_date} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md"/>
                        </div>
                        <div>
                            <label htmlFor="due_date">Due Date</label>
                            <input type="date" name="due_date" id="due_date" value={formData.due_date} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md"/>
                        </div>
                    </div>
                    <div>
                        <label htmlFor="assignee_id">Assign To</label>
                        <select name="assignee_id" id="assignee_id" value={formData.assignee_id} onChange={handleChange} disabled={taskData?.is_commissioned} className="mt-1 block w-full rounded-md">
                            <option value="">-- Unassigned --</option>
                            {users.map(u => ( <option key={u.id} value={u.id}> {u.full_name || u.email} ({u.role}) </option> ))}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="predecessors" className="block text-sm font-medium">Depends On (Predecessors)</label>
                        <Select
                            id="predecessors" isMulti options={potentialPredecessors} value={formData.predecessors}
                            onChange={handlePredecessorChange} className="mt-1" classNamePrefix="react-select"
            ...
            {canCommissionTask && taskData?.status === "Done" && !taskData.is_commissioned && (
                <div className="max-w-lg mt-4 mb-8 flex justify-end">
                    <button onClick={handleCommissionTask} disabled={isCommissioning} className="px-6 py-2 border rounded-md shadow-sm text-white bg-teal-600 hover:bg-teal-700">
