...
// Final version with granular permissions and Tenant dropdown for SuperAdmins.
...
function UserEditPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        is_superuser: false,
        hourly_rate: '',
        tenant_id: '', // Added for tenant dropdown
    });
    ...
    // State for Project Assignments & Tenant List
    const [allProjects, setAllProjects] = useState([]);
    const [allTenants, setAllTenants] = useState([]); // For tenant dropdown
    ...
    const fetchPageData = useCallback(async () => {
        if (!authIsLoading && isAuthenticated && canViewPage && userId) {
            ...
            setError('');
            try {
                // Initial promises for user and projects
                const promises = [
                    axiosInstance.get(`/users/${userId}`),
                    axiosInstance.get('/projects/')
                ];

                // If the editor is a superuser, also fetch all tenants
                ...
                // Fetch project memberships for the user being edited
    ...
    const handleSubmitUserDetails = async (e) => {
        ...
        // Logic to build payload of only changed fields
        ...
        try {
            ...
            setInitialUserData(response.data); // Resync with the saved data
        } catch (err) {
    ...
    const handleAdminResetPasswordSubmit = async (e) => {
        ...
        if (currentUser && currentUser.id === parseInt(userId, 10)) { 
            toast.error("Use Account Settings for own password."); 
            return; 
        }
                ...
                <div className="space-y-8">
                    <fieldset disabled={!canSaveChanges || isSubmittingUserDetails}>
                        <form onSubmit={handleSubmitUserDetails} className="space-y-4 bg-white dark:bg-gray-800 p-6 rounded shadow-md">
                            ...
                            <div> <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email <span className="text-red-500">*</span></label> <input type="email" name="email" id="email" required value={formData.email} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/> </div>
                            <div> <label htmlFor="full_name" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Full Name</label> <input type="text" name="full_name" id="full_name" value={formData.full_name} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/> </div>
                            <div> <label htmlFor="employee_id" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Employee ID</label> <input type="text" name="employee_id" id="employee_id" value={formData.employee_id} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/> </div>
                            <div> <label htmlFor="kennitala" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Kennitala</label> <input type="text" name="kennitala" id="kennitala" value={formData.kennitala} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/> </div>
                            <div> <label htmlFor="phone_number" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Phone Number</label> <input type="tel" name="phone_number" id="phone_number" value={formData.phone_number} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/> </div>
                            <div> <label htmlFor="location" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Location</label> <input type="text" name="location" id="location" value={formData.location} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/> </div>
                            
                            {canSaveChanges && (
                                <div>
                                    <label htmlFor="hourly_rate" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Hourly Rate (€)</label>
                                    <input type="number" name="hourly_rate" id="hourly_rate" value={formData.hourly_rate} onChange={handleChange} step="0.01" placeholder="e.g., 50.50" className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm"/>
                                </div>
                            )}

                            <div> <label htmlFor="role" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Role <span className="text-red-500">*</span></label> <select name="role" id="role" required value={formData.role} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm">{ROLES_LIST.map(r => (<option key={r} value={r}>{r.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</option>))}</select> </div>
                            
                            {isSuperuserEditing && !formData.is_superuser && (
                                <div>
                                    <label htmlFor="tenant_id" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Tenant</label>
                                    <select name="tenant_id" id="tenant_id" value={formData.tenant_id || ''} onChange={handleChange} className="mt-1 block w-full rounded-md shadow-sm">
                            ...
                            <div className="flex items-center"> <input type="checkbox" name="is_active" id="is_active" checked={formData.is_active} onChange={handleChange} className="h-4 w-4 text-indigo-600 rounded"/> <label htmlFor="is_active" className="ml-2 block text-sm text-gray-900 dark:text-gray-300">Active User</label></div>
                            
                            {isSuperuserEditing && (
                                <div className="flex items-center">
                                    <input type="checkbox" name="is_superuser" id="is_superuser" checked={formData.is_superuser} onChange={handleChange} disabled={currentUser.id === parseInt(userId, 10)} className="h-4 w-4 text-indigo-600 rounded"/>
                                    <label htmlFor="is_superuser" className="ml-2 block text-sm text-gray-900 dark:text-gray-300">Superuser</label>
                    ...
                    {canSaveChanges && currentUser.id !== parseInt(userId, 10) && (
                         <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-md">
                             ...
                             <form onSubmit={handleAdminResetPasswordSubmit} className="space-y-4">
                                 <div><label htmlFor="newPasswordByAdmin">New Password</label><input type="password" name="newPasswordByAdmin" required minLength="8" value={newPasswordByAdmin} onChange={(e) => setNewPasswordByAdmin(e.target.value)} disabled={isSubmittingNewPassword} className="mt-1 block w-full rounded-md shadow-sm" /></div>
                                 <div><label htmlFor="confirmNewPasswordByAdmin">Confirm New Password</label><input type="password" name="confirmNewPasswordByAdmin" required minLength="8" value={confirmNewPasswordByAdmin} onChange={(e) => setConfirmNewPasswordByAdmin(e.target.value)} disabled={isSubmittingNewPassword} className="mt-1 block w-full rounded-md shadow-sm" /></div>
                ...
                <div>
                    {projectDataLoading ? (
                        ...
                        <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-md">
                            ...
                            {allProjects.length === 0 ? <p className="text-sm text-gray-500">No projects available.</p> : (
                                <div className="space-y-2 max-h-96 overflow-y-auto">
                                    {allProjects.map(project => (
                                        <div key={project.id} className="flex items-center">
                                            <input type="checkbox" id={`project-assign-${project.id}`} checked={assignedProjectIds.has(project.id)} onChange={(e) => handleProjectAssignmentChange(project.id, e.target.checked)} disabled={!canSaveChanges} className="h-4 w-4 text-indigo-600 rounded"/>
