...
async def create_new_inventory_item(request: Request, item: schemas.InventoryItemCreate, db: DbDependency, current_user: ManagerOrAdminDependency):
...
@limiter.limit("100/minute")
async def read_single_inventory_item(request: Request, item_id: int, db: DbDependency, current_user: CurrentUserDependency):
...
@limiter.limit("100/minute")
async def update_existing_inventory_item(request: Request, item_id: int, item_update: schemas.InventoryItemUpdate, db: DbDependency, current_user: ManagerOrAdminDependency):
...
@limiter.limit("100/minute")
async def update_item_needed_quantity(request: Request, item_id: int, needed_qty_data: schemas.InventoryItemUpdateNeededQty, db: DbDependency, current_user: TeamLeaderOrHigher):
...
@limiter.limit("100/minute")
async def delete_existing_inventory_item(request: Request, item_id: int, db: DbDependency, current_user: ManagerOrAdminDependency):
